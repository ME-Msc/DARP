...
- 当前 requirement 基线只支持 `reward-deterministic`：会检查 reward 中不能调用随机分布；其它 requirements 会后续逐个实现和提交。
...
- [x] Phase 4：通用 RDDL 问题建模
  ...
  - [x] 4.5：实现 `reward-deterministic` requirement 基线，后续 requirements 单独实现、测试和提交
...
- 当前 requirement 语义检查只支持 `reward-deterministic`；其它 requirements 已在 `rddl/semantics.py` 中记录为逐步实现计划。
