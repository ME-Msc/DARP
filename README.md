...
- 当前 requirement 基线支持 `reward-deterministic` 和 `cpf-deterministic`：分别检查 reward 与 state-fluent transition CPF 中不能调用随机分布；其它 requirements 会后续逐个实现和提交。
...
│       ├── factored_door_domain.rddl # Phase 4 factored-state 示例，包含随机 CPF 和 observ-fluent。
...
- 当前 requirement 语义检查支持 `reward-deterministic` 和 `cpf-deterministic`；其它 requirements 已在 `rddl/semantics.py` 中记录为逐步实现计划。
